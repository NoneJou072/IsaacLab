...
class MiHandOverRGBCameraEnv(DirectMARLEnv):
    cfg: MiHandOverRGBCameraEnvCfg

    def __init__(self, cfg: MiHandOverRGBCameraEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print(self.right_hand.joint_names)
        ...
        self.goal_pos[:, :] = torch.tensor([0.0, -0.6, 0.54], device=self.device)
        ...
        observations = {
            "right_hand": torch.cat(
                (
                    ...
                    # tiled camera data
                    self.camera_data.view(self.num_envs, -1),
                ),
                dim=-1,
            ),
            "left_hand": torch.cat(
    ...
    def _compute_intermediate_values(self):
        ...
        if "rgb" in self.cfg.tiled_camera.data_types:
            self.camera_data = self._tiled_camera.data.output[data_type] / 255.0
            # normalize the camera data for better training results
            mean_tensor = torch.mean(self.camera_data, dim=(1, 2), keepdim=True)
            self.camera_data -= mean_tensor
        elif "depth" in self.cfg.tiled_camera.data_types:
            self.camera_data = self._tiled_camera.data.output[data_type]
            self.camera_data[self.camera_data == float("inf")] = 0
        if self.cfg.write_image_to_file and self.common_step_counter % 500 == 0:
            save_images_to_file(self.camera_data.clone(), f"mihand_{data_type}.png")
